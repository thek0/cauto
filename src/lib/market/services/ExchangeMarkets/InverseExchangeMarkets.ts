...
export class InverseExchangeMarkets implements IExchangeMarkets {
    ...
    static async at(exchange: Exchange): Promise<InverseExchangeMarkets> {
        ...
        var spotMarkets = allMarkets
            .filter((m: Market) => {
                return m?.inverse === true && m.expiry === undefined
            })
            .map((m: Market) => new MarketData(m, exchangeInfo))

        return new InverseExchangeMarkets(spotMarkets, exchangeInfo)
    }
