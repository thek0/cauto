...
export class SpotExchangeMarkets implements IExchangeMarkets {
    ...
    static async at(exchange: Exchange): Promise<SpotExchangeMarkets> {
        ...
        var spotMarkets = allMarkets
            .filter((m: Market) => {
                return m?.spot === true && m.expiry === undefined
            })
            .map((m: Market) => new MarketData(m, exchangeInfo))

        return new SpotExchangeMarkets(spotMarkets, exchangeInfo)
    }
